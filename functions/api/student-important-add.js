...
export async function onRequest(context) {
  ...
  const importantAdderRole = (profile ? profile.role || '' : '').toLowerCase();
  if (!profile || !['student', 'admin'].includes(importantAdderRole)) return json({ error: 'Access denied' }, 403);
  ...
  const { error } = await admin
    ...
    .upsert([{ user_id: user.id, mcq_id: mcqId }], {
  ...
  if (error) {
    ...
    return json({ error: error.message || 'Failed to add important item' }, 500);
  }
  return json({ ok: true, message: 'Marked as Important' });
}
